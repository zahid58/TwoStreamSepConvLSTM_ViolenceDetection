...
def getPose(image, e=None):
  # add cropping handling here
  e.inference(image, resize_to_default = False, upsample_size= 8)
  out1 = np.amax(e.heatMat[:, :, :-1], axis=2)
  out1 = cv2.normalize(out1, None, 0, 255, cv2.NORM_MINMAX)
  out1 = cv2.blur(out1,(15,15))
  tmp = e.pafMat.transpose((2, 0, 1))
  out2 = np.amax(np.absolute(tmp[::2, :, :]), axis=0)
  out3 = np.amax(np.absolute(tmp[1::2, :, :]), axis=0)
  out2 = cv2.normalize(out2, None, 0, 255, cv2.NORM_MINMAX)
  out2 = cv2.blur(out2,(15,15))
  out3 = cv2.normalize(out3, None, 0, 255, cv2.NORM_MINMAX)
  out3 = cv2.blur(out3,(15,15))
  out = out1 + out2 + out3
  out = cv2.normalize(out, None, 0, 255, cv2.NORM_MINMAX)
  #here we might even pass the three channels stacked
  return out


def save_figures_from_video(dataset_video_path, video_filename, suffix, figures_path, poses_path, skip_frames = 25,apply_norm = True, apply_diff = True, fix_len = None,crop_x_y =None, figure_shape = None, e=None):
    seq_len = 0

    video_figures_path = os.path.join(figures_path ,video_filename)
    video_poses_path = os.path.join(poses_path, video_filename)

    if not os.path.exists(video_figures_path):
        os.makedirs(video_figures_path)
    if not os.path.exists(video_poses_path):
        os.makedirs(video_poses_path)
    ...
    while success:
        ...
        if seq_len % skip_frames == 0:
            if success:
                x_ = figure
                if ( crop_x_y is not None ) and (figure_shape is not None):
                  figure_curr = crop_img__remove_Dark(x_,crop_x_y[0],crop_x_y[1],x_.shape[0],x_.shape[1],figure_shape)
                else:
                  figure_curr = x_
                image_file = os.path.join(video_figures_path , "frame_%d.jpg" % seq_len)
                pose_file = os.path.join(video_poses_path , "frame_%d.jpg" % seq_len)
                pose_curr = getPose(figure_curr, e=e)
                imgfiles.append(image_file)
                posefiles.append(pose_file)
                cv2.imwrite(image_file, figure_curr)
                cv2.imwrite(pose_file,pose_curr)

        seq_len += 1
    video_images = dict(images_path = (video_figures_path , video_poses_path), name = video_filename,
                        images_files = imgfiles, poses_files = posefiles, sequence_length = seq_len, label = label)

    return video_images


def createDataset(datasets_video_path, figure_output_path, pose_output_path ,fix_len, crop_x_y = None, force = False,figure_shape=None, e=None):
    videos_seq_length = []
    datasets_images = {}
    videos_frames_paths = []
    videos_labels = []
    ...
    for dataset_name, dataset_video_path in datasets_video_path.items():
        ...
        dataset_poses_path = os.path.join(pose_output_path, dataset_name)
        if not os.path.exists(dataset_figures_path):
            os.makedirs(dataset_figures_path)
        if not os.path.exists(dataset_poses_path):
            os.makedirs(dataset_poses_path)
        dataset_images = []
        cnt = 1
        for filename in os.listdir(dataset_video_path):
            print('file number:', cnt, filename[0:8])
            cnt = cnt +1
            if filename.endswith(".avi") or filename.endswith(".mpg"):
                ...
                if os.path.isfile(video_images_file) and not force:
                    ...
                    video_images = save_figures_from_video(dataset_video_path, filename[:-4],filename[-4:], dataset_figures_path, dataset_poses_path, fix_len = fix_len, crop_x_y = crop_x_y,figure_shape= figure_shape, e=e)
...
def test_data_integrity(data_paths,labels,batch_size,figure_shape,seq_length,use_aug,use_crop,crop_x_y,classes = 1):
    print('conducting data integrity test on ',len(data_paths),' files')
    errCount = 0
    for i in range(len(data_paths)):
        ...
        try: 
          ...
          if(X.ndim!=5 or P.ndim!=5):
            print('dimensions not ok')
            print('X.shape :',X.shape, ' P.shape:',P.shape )
            errCount += 1
        except:
          print('XXXXXXX get_sequence NOT ok')
          errCount += 1
...
def crop_img__remove_Dark(img,x_crop,y_crop,y,x,figure_size):
    x_start = x_crop
    x_end = x - x_crop
    y_start = y_crop
    y_end = y-y_crop
    ...
    return cv2.resize(temp,(figure_size,figure_size))
...
def get_sequences(data_paths,labels,figure_shape,seq_length,classes=1, use_augmentation = False,use_crop=True,crop_x_y=None):
    ...
    #print('loop starts..')
    cnt = 0
    for frame_pose_path, label in zip(data_paths,labels):
        #print('loop', i)
        ...
        #print('loading poses')
        p = frame_loader(poses, figure_shape)
        #if(crop_x_y):
        ...
        if use_augmentation:
            ...
            if rand > 0.5:
                if(use_crop):
                    ...
                    p = [crop_img(p_,figure_shape,0.7,corner) for p_ in p]
                x = [frame.transpose(1, 0, 2) for frame in x]
                p = [pose.transpose(1, 0, 2) for pose in p]
                if(Debug_Print_AUG):
    ...
    P = pad_sequences(P, maxlen=seq_length, padding='pre', truncating='pre')
    if classes > 1:
